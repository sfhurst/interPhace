...
const chordPresets = data.CHORD_PRESET_TABLE;
assert(Array.isArray(chordPresets) && chordPresets.length >= 33, "Harmonic preset bank must contain at least the original 33 entries");
const chordNames = new Set();
for (let index = 0; index < (chordPresets || []).length; index += 1) {
  const preset = chordPresets[index];
  assert(preset.index === index, `Harmonic preset ${preset.name || index} must use contiguous index ${index}`);
  assert(preset.name && !chordNames.has(preset.name), `Invalid or duplicate harmonic preset name: ${preset.name}`);
  chordNames.add(preset.name);
...
assert(chordSliderMatch && Number(chordSliderMatch[1]) === chordPresets.length - 1, "Chord preset slider max must match preset count");
