...
window.GraphBuilder = {
  build(ctx, patch, plan = RenderPlan.create(patch, ctx.sampleRate), options = {}) {
    ...
    if (typeof TextureEngine !== "undefined") {
      ...
      postEffectsBus = texture.postEffectsBus || texture.postFxNode || null;
    }
    ...
    if (postEffectsBus) {
      const rackMix = ctx.createGain();
      mainBus.connect(rackMix);
      postEffectsBus.connect(rackMix);
      mainBus = rackMix;
    }
